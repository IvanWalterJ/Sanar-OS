-- ============================================================================
-- admin_migrate_profile: permite a admins actualizar el perfil ADN de un
-- cliente (campos de migración) bypassando RLS. Sin esta RPC, el UPDATE
-- directo desde el cliente Supabase es silenciosamente filtrado por RLS
-- (afecta 0 filas sin error), lo que hace que el MigrationWizard parezca
-- exitoso pero no persista nada.
-- ============================================================================
...
BEGIN
  ...
  -- UPDATE dinámico via jsonb_populate_record + merge
  UPDATE profiles
     SET
...
COMMENT ON FUNCTION admin_migrate_profile(UUID, JSONB)
  IS 'Admin-only: actualiza campos de migración/ADN del perfil de un cliente. Bypass RLS con whitelist de campos permitidos.';
