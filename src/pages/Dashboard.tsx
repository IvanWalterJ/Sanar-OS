...
function getTypeBadge(tipo?: string) {
  switch (tipo) {
    case 'VIDEO': return 'bg-[#C8893A]/15 text-[#C8893A] border-[#C8893A]/25';
    case 'HERRAMIENTA': return 'bg-[#2DD4A0]/15 text-[#2DD4A0] border-[#2DD4A0]/25';
    case 'COACH': return 'bg-[#F0EAD8]/10 text-[#F0EAD8]/70 border-[#F0EAD8]/15';
    case 'AGENTE': return 'bg-purple-500/15 text-purple-400 border-purple-500/25';
    default: return 'bg-[#F0EAD8]/5 text-[#F0EAD8]/50 border-[#F0EAD8]/10';
  }
}
...
function MetricCard({ label, value, sub }: { label: string; value: string; sub: string }) {
  return (
    <div className="card-panel p-5">
      <p className="text-[10px] text-[#F0EAD8]/40 uppercase tracking-widest mb-2 font-semibold">{label}</p>
      <p className="text-2xl font-light text-[#F0EAD8] tracking-tight">{value}</p>
      <p className="text-xs text-[#F0EAD8]/50 mt-1">{sub}</p>
    </div>
  );
}
...
export default function Dashboard({ setCurrentPage, userId }: { setCurrentPage: (page: string) => void, userId?: string }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden card-panel p-8 border border-[#C8893A]/20 bg-gradient-to-br from-[#C8893A]/[0.05] to-transparent">
        <div className="absolute top-0 right-0 w-64 h-64 bg-[#C8893A]/10 blur-[100px] rounded-full" />
        <div className="relative z-10">
          <p className="text-2xl font-light text-[#F0EAD8] mb-2" style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>Buenos días, {nombreDisplay}.</p>
          <p className="text-sm text-[#F0EAD8]/60 max-w-lg mb-6 leading-relaxed">
            Estás en la <strong className="text-[#F0EAD8]/90">Semana {data.semanaActual} de 13</strong>. Tenés <strong className="text-[#C8893A]">{data.tareasHoy.length} {data.tareasHoy.length === 1 ? 'tarea pendiente' : 'tareas pendientes'}</strong> para avanzar con tu ADN.
          </p>
          <button onClick={() => setCurrentPage('roadmap')} className="text-[11px] font-bold text-[#C8893A] hover:text-[#D9A04E] transition-colors flex items-center gap-1.5 uppercase tracking-widest bg-[#C8893A]/10 px-4 py-2 rounded-lg border border-[#C8893A]/20 w-max">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 card-panel p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-[11px] font-bold text-[#F0EAD8] tracking-widest uppercase">Tu foco de hoy</h2>
            <button onClick={() => setCurrentPage('roadmap')} className="text-[10px] text-[#F0EAD8]/40 hover:text-[#C8893A] uppercase font-bold tracking-wider transition-colors">
              Ir a tareas →
            </button>
          </div>
          ...
          <div className="space-y-3">
            {data.tareasHoy.length === 0 ? (
              <div className="py-10 text-center border border-dashed border-[rgba(200,137,58,0.15)] rounded-xl bg-[#241A0C]/30">
                <CheckCircle2 className="w-8 h-8 text-[#2DD4A0]/50 mx-auto mb-3" />
                <p className="text-sm text-[#F0EAD8]/60">Todo al día. Estás libre.</p>
                <p className="text-xs text-[#F0EAD8]/30 mt-1">Revisá tu hoja de ruta para ver los próximos pilares.</p>
              </div>
            ) : data.tareasHoy.map((t, idx) => (
              <div
                key={idx}
                className="group flex items-start gap-4 p-4 rounded-xl bg-[#241A0C]/30 border border-[rgba(200,137,58,0.1)] hover:bg-[#241A0C]/60 hover:border-[rgba(200,137,58,0.25)] transition-all cursor-pointer"
                onClick={() => setSelectedTask(t)}
              >
                <div className="shrink-0 mt-0.5">
                  <div className="w-5 h-5 rounded-full border border-[#F0EAD8]/20 group-hover:border-[#C8893A] transition-colors flex items-center justify-center">
                    <div className="w-1.5 h-1.5 bg-transparent group-hover:bg-[#C8893A] rounded-full" />
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[#F0EAD8]/90">{t.titulo}</p>
                  <p className="text-[10px] text-[#F0EAD8]/40 mt-1">{t.pilarTitulo}</p>
                  <div className="flex flex-wrap items-center gap-3 mt-3">
                    ...
                    <span className="text-[10px] text-[#F0EAD8]/40 flex items-center gap-1 font-medium">
                      <Clock className="w-3 h-3" /> {t.tiempo_estimado || '15–30 min'}
                    </span>
                    {(t.herramienta_id || t.agente_id) && (
                      <span className="text-[9px] text-[#C8893A] font-bold uppercase tracking-wider">Ver herramienta →</span>
                    )}
                  </div>
                </div>
              </div>
        ...
        {/* Próximo Hito (40%) */}
        <div className="lg:col-span-5 card-panel p-6 relative overflow-hidden flex flex-col justify-between border-[#C8893A]/15 bg-gradient-to-br from-[#C8893A]/[0.03] to-transparent">
          <div className="absolute -top-10 -right-10 w-40 h-40 bg-[#C8893A]/10 blur-[50px] rounded-full" />

          {proximoHito ? (
            <>
              <div>
                <h2 className="text-[11px] font-bold text-[#C8893A] tracking-widest uppercase mb-6 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-[#C8893A] animate-pulse ring-4 ring-[#C8893A]/20" /> Próximo hito
                </h2>
                <p className="text-xl font-medium text-[#F0EAD8] mb-3 line-clamp-2 leading-tight" style={{ fontFamily: 'var(--font-display)', fontStyle: 'italic' }}>{proximoHito.titulo}</p>
                <p className="text-xs text-[#F0EAD8]/50 flex items-center gap-1.5">
              ...
              <div className="mt-10">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-[10px] text-[#F0EAD8]/50 font-bold uppercase tracking-wider">Progreso del hito</span>
                  <span className="text-[10px] text-[#F0EAD8] bg-[#F0EAD8]/10 px-2 py-0.5 rounded-full">
                    {tareasRestantesHito === 0 ? 'Completado' : `Faltan ${tareasRestantesHito} ${tareasRestantesHito === 1 ? 'tarea' : 'tareas'}`}
                  </span>
                </div>
                <div className="h-2 bg-[#F0EAD8]/5 rounded-full overflow-hidden">
                  <div className="h-full bg-[#C8893A] rounded-full transition-all duration-500" style={{ width: `${pctHito}%` }} />
                </div>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center justify-center h-full text-center py-6">
              ...
              <p className="text-xs text-[#F0EAD8]/40 mt-1 max-w-[180px]">Completaste todos los pilares del programa.</p>
            </div>
          )}
        </div>
      </div>
